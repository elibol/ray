...
int main(int argc, char *argv[]) {
  ...
  ray::Raylet server1(io_service, io_service, std::string(argv[1]), resource_config, om_config, mock_gcs_client);
  om_config.store_socket_name = store2;
  ray::Raylet server2(io_service, io_service, std::string(argv[2]), resource_config, om_config, mock_gcs_client);
  io_service.run();
}
